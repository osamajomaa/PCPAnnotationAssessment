...
def create_ref_file(pmid_refs, species, resnum):
    fHandler = open(os.path.join(CURR_PATH,"SCOPUS_API_Files/"+species+"/Results/data/res"+resnum), 'wb')
    cPickle.dump(pmid_refs, fHandler)
    fHandler.close()

def create_err_file(paper_errors, species, errnum):
    ...
    file = open(os.path.join(CURR_PATH,"SCOPUS_API_Files/"+species+"/Results/errors/err"+errnum), 'wb')
    file.write(err_file)
    file.close()
...
if __name__ == "__main__":
    ...
    create_err_file(paper_errors, species, rqnum)
